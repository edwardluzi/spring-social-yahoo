...
public class DetailQuoteTemplate extends AbstractTemplate implements DetailQuoteOperations
{
    ...
    public DetailQuoteTemplate(RestOperations restOperations, ObjectMapper objectMapper, boolean isAuthorized)
    {
    ...
    public List<DetailQuote.Quote> getQuotes(Collection<String> symbols)
    {
        ...
        if (symbols.size() > 1)
        {
            symbolsString = String.join(",",
                    symbols.stream().map(s -> String.format("\"%s\"", s)).collect(Collectors.toList()));
        }
        else
        {
            ...
            symbolsString = String.join(",",
                    extras.stream().map(s -> String.format("\"%s\"", s)).collect(Collectors.toList()));
        }
        ...
        DetailQuote.Wrapper wrapper = getRestOperations().getForObject(buildUri(params), DetailQuote.Wrapper.class);

        List<DetailQuote.Quote> quotes = null;

        try
        {
            DetailQuote.Query queryObject = wrapper.getQuery();
